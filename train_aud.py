...
parser.add_argument('--x_image_shape', type=int, default=1024, 
...
# Effnet - 1280
# VGG - 512
parser.add_argument('--x_sound_shape', type=int, default=1024, #remeber change in mafnet aud also
...
def train_model(train_data, val_data):
    ...
    # setup network
    MAFnet = model.MAFnet_aud(x_sound, y,  args.x_sound_shape, args.n_hidden, args.n_classes)
    ...
    f=open('final_audio_only'+args.pick+'.txt','w')
    f2=open('final_audio_tpr'+args.pick+'.txt','w')
    f3=open('final_audio_fpr'+args.pick+'.txt','w')
    ...
    for n in range(args.n_epoch):
        ...
        for i in range(int(N / args.batch_size)):
            ...
            feed_dict = {x_sound: train_data[1][i*args.batch_size:(i+1)*args.batch_size],
...
def test_model(test_data):
    ...
    f=open('ypred_audio_only'+args.pick+'.txt','w')
    f2=open('yactual_audio_only'+args.pick+'.txt','w')

    saver = tf.compat.v1.train.Saver()
    saver.restore(sess, os.path.join('model', 'MAFnet'))
    accuracy,ymat,ypred,yactual= sess.run(MAFnet.acc, feed_dict={x_sound: test_data[1],
                                               y: test_data[2],
                                               MAFnet.isTraining: False})
    ac=accuracy*100
    ac=round(ac,2)
